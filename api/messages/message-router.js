...
// CREATE STREAM FILE
router.post("/stream", auth.auth, (req, res) => {
...
router.post("/train", (req, res) => {
    ...
    Users.findUser(email)
        ...
        .then(() => {
            const file = fs.createWriteStream(`./stream/allEmails${DsUser}Search.file`);
            Messages.emails(DsUser)
                ...
                .then(() => {
                    ...
                    const src = fs.createReadStream(`./stream/allEmails${DsUser}Search.file`);
...
router.post("/", auth.auth, async (req, res) => {
    try {
        ...
        emails
            ? res.status(200).json({ allEmailsFetched: { fetched: true, date: Date.now() } })
            : res.status(400).json({ fetched: false, msg: "Emails failed to fetch." });
    } catch (err) {
        res.status(500).json({ fetched: false, err, msg: "The entire request failed." });
    }
});
